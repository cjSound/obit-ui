...
export default {
    ...
    render(h) {
        ...
        this.oldChildLength = item[this.childenname] == null ? 0 : item[this.childenname].length;
        return (
            <div>
                <div class={{ 'tab-row': true, 'last-child': item[this.childenname] == null || item[this.childenname].length == 0 }}>
                    <div class="row-item" onClick={() => this.showTog()} style={{ 'width': widthArray[0] }}  >
                        <span style={{ 'padding-left': this.left + 'px' }}></span>
                        {
                            item[this.childenname] == null || item[this.childenname].length == 0 ? <span>&nbsp;</span> :
                                <span class={{
                                    'icon iconfont': true,
                ...
                {
                    (item[this.childenname] != null && item[this.childenname].length) > 0 ?
                        <tableBody class={{ 'tabody': true, 'ishidden': !this.open }} slotMap={this.slotMap}
                            dataList={item[this.childenname]}
